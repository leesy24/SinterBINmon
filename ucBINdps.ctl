...
Private Sub picGET_Click()
    ...
    maxyrange = 20000
    minyrange = 0
    maxxrange = 5000  '''BIN5:: 7000
    minxrange = -5000  '''BIN5:: -3000
  ...
  If (UCindex = 6) Or (UCindex = 7) Or (UCindex = 16) Or (UCindex = 17) Then
    picGET.Line (minxrange + 2500, maxyrange - 500)-(minxrange + 2500, maxyrange * Val(txtOpMid))
    picGET.Line (maxxrange - 2500, maxyrange - 500)-(maxxrange - 2500, maxyrange * Val(txtOpMid))
    ''
    picGET.Line (minxrange + 2500, maxyrange * Val(txtOpMid))-(-600, minyrange - 100)
    picGET.Line (maxxrange - 2500, maxyrange * Val(txtOpMid))-(600, minyrange - 100)
  Else
...
Public Sub setBinID()
    If UCindex < 6 Then
        cmdCONN.Caption = UCindex + 1 & ") " & "3소결BIN-" & (UCindex + 1)  ''0~5::1소결
    ElseIf UCindex < 10 Then
        cmdCONN.Caption = UCindex + 1 & ") " & "3소결BIN-" & (UCindex + 2)  ''6~9::1소결
    ElseIf (UCindex > 9) And (UCindex < 16) Then
        cmdCONN.Caption = UCindex + 1 & ") " & "4소결BIN-" & (UCindex - 10 + 1) ''10~15::2소결
    ElseIf (UCindex > 15) And (UCindex < 20) Then
        cmdCONN.Caption = UCindex + 1 & ") " & "4소결BIN-" & (UCindex - 10 + 2) ''16~19::2소결
    End If
